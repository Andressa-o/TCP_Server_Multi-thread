...
def threaded(c, addr, dir):
    ...
    # option 2: download a file
        ...
        if file_management.file_Exist(dir + res2) or res2 in cache:
            # checks if file is larger than the cache
            if file_management.getFile_size(dir + res2) > CACHE_SIZE and res2 not in cache:
                ...
                if res2 not in cache:
                    ...
                    print("|LOG|: |CACHE| [port: %s] add file '%s' on cache" % (
                        addr[1], res2))
                    size_file = file_management.getFile_size(dir + res2)
